...
export function SegmentChart({ data }: { data: { name: string; total: number }[] }) {
  return (
    <div className="h-64 rounded-lg border border-gold-100 bg-white p-4">
      <p className="mb-2 text-sm font-medium text-ink-700">Faturamento por segmento</p>
      <ResponsiveContainer width="100%" height="85%">
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#F5E7C4" />
...
export function PaymentMethodChart({ data }: { data: { name: string; total: number }[] }) {
  return (
    <div className="h-64 rounded-lg border border-gold-100 bg-white p-4">
      <p className="mb-2 text-sm font-medium text-ink-700">Faturamento por forma de pagamento</p>
      <ResponsiveContainer width="100%" height="85%">
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="#F5E7C4" />
  ...
  return (
    <div className="rounded-lg border border-gold-100 bg-white p-4">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-sm font-medium text-ink-700">Concentração Boleto / Recorrente</p>
        <span className="text-lg font-medium" style={{ color: barColor }}>
          {pct.toFixed(1)}%
        </span>
      </div>
      <div className="h-3 w-full overflow-hidden rounded-full bg-gold-50">
        <div
